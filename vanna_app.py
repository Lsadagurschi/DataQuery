...
# Inicializar cliente Vanna
def init_vanna_client(api_key):
    try:
        vanna_client = VannaDefault(api_key=api_key)
        return vanna_client
...
# Função para gerar SQL com Vanna.ai
def generate_vanna_sql(query, vanna_client):
    try:
        sql = vanna_client.generate_sql(query)
        return sql
    except Exception as e:
...
def main():
                ...
                if query:
                    with st.spinner("Gerando SQL..."):
                        if st.session_state.vanna_client and not st.session_state.is_demo:
                            ...
                            sql = generate_vanna_sql(query, None)
                ...
                if "current_results" in st.session_state and st.session_state.current_results is not None:
                    ...
                    if len(df.columns) >= 2:
                        ...
                        with col2:
                            ...
                            y_default = numeric_cols.index(y_col) if y_col in numeric_cols else 0
                            y_axis = st.selectbox("Eixo Y", y_options, index=y_default if y_options else 0)
    ...
    # Rodapé
    st.divider()
    st.write("DataTalk com Vanna.ai © 2025 - Consulta avançada de banco de dados por linguagem natural")
